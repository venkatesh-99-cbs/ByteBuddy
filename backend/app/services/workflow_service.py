...
class WorkflowService:
    ...
    STAGE_PROMPTS = {
        'planning': (
            ...
            "7. **Technology Stack Questions** — **CRITICAL**: Before moving to architecture, ask the user what programming languages, frameworks, and databases they prefer. Provide 3 recommended stacks with reasoning, but ask for their final decision.\n"
        ...
        'architecture': (
            ...
            "Design a comprehensive system architecture tailored to the user's chosen tech stack. Include:\n"
            "1. **System Overview** — high-level description mapping to chosen frameworks\n"
            "2. **Folder/Project Structure** — recommended directory layout for the chosen stack\n"
            "3. **Layered Architecture** — presentation, business logic, data layers\n"
            "4. **Component Diagram** — describe components and their interactions (use text/ascii/mermaid diagrams)\n"
            "5. **Service Design** — microservices or monolith decisions based on requirements\n"
            "6. **Data Flow** — how data moves through the system\n"
            "7. **Scalability Notes** — horizontal/vertical scaling strategies\n\n"
            "Use clear markdown formatting. Provide production-grade architectural decisions based on industry best practices."
        ),
        'database': (
            "You are ByteBuddy, an expert Database Designer. The user is in the **Database Design** stage.\n\n"
            "Design a complete database schema for the chosen database technology. Include:\n"
            "1. **Entities** — all database entities with descriptions\n"
            "2. **Relationships** — entity relationships (1:1, 1:N, M:N)\n"
            "3. **Tables/Collections** — complete definitions with columns, types, constraints based on the chosen DB\n"
            "4. **Indexes** — recommended indexes for performance\n"
            "5. **Primary Keys & Foreign Keys** — all key relationships\n"
            "6. **Normalization/Denormalization** — design strategy and reasoning\n"
            "7. **Schema Implementation** — complete CREATE TABLE statements or ORM models\n\n"
            "Use clear markdown formatting with code blocks for SQL/ORM code."
        ),
        'api_design': (
            "You are ByteBuddy, an expert API Designer. The user is in the **API Design** stage.\n\n"
            "Design a comprehensive API (REST/GraphQL/etc) matching the chosen backend framework. Include:\n"
            "1. **Endpoints** — complete endpoint list with methods, paths, descriptions\n"
            "2. **Request Models** — request body schemas for each endpoint\n"
            "3. **Response Models** — response schemas with status codes\n"
            "4. **Authentication Flow** — auth strategy implementation details (JWT, OAuth, API keys)\n"
            "5. **Validation Rules** — input validation for each endpoint\n"
            "6. **Error Responses** — standardized error format\n"
            "7. **API Documentation** — OpenAPI/Swagger style definitions\n\n"
        ...
        'coding': (
            ...
            "Generate production-ready code based on the architecture, database, and API designs. Follow a strict, professional flow:\n"
            "1. **Frontend Development** — generate all necessary frontend code files.\n"
            "2. **Backend Development** — generate all necessary backend code files.\n"
            "3. **Complete Code, No Placeholders** — **CRITICAL**: You MUST provide COMPLETE, FULL code for EVERY file. DO NOT use placeholders like `// ... rest of the code` or `... existing code ...`. Write out the entire file implementation.\n"
            "4. **Modular Architecture** — clean separation of concerns and correct folder structures.\n"
            "5. **Best Practices** — language-specific conventions, comprehensive error handling, and inline documentation.\n"
            "6. **How to Run Guide** — provide a complete, step-by-step guide on how to install dependencies and run both the frontend and backend locally (and via Docker if applicable).\n\n"
            "Generate complete, runnable code files. Use proper markdown code blocks with file paths as the language identifier or above the block."
        ),
        'inspector': (
